...
import os
import sys  # Ajout pour relancer main.py

def return_to_main():
    """Relancer main.py quand l'utilisateur appuie sur une touche."""
    print(f"{BEFORE + current_time_hour() + AFTER} {INFO} Appuyez sur 'Entrée' pour revenir au menu principal.")
    ...
    links = {
        ...
        "Market": {
            ...
            "Drug Stores": {
                ...
                "Smokeables": "http://kl4gp72mdxp3uelicjjslqnpomqfr5cbdd3wzo5klo3rjlqjtzh5fcyd.onion/",
            },
            "The Pastry Shop": "http://5dy5ltbfef4fj2g2ntfukdsam45uxag75jylhfgmrpqtohe7zdf7igqd.onion/",
        }
    }

    for category, urls in links.items():
        print(f"{BEFORE + current_time_hour() + AFTER} {INFO} {category} Links:")
        for site_name, url in urls.items():
            if isinstance(url, dict):
                for sub_site_name, sub_url in url.items():
                    print(f"{BEFORE + current_time_hour() + AFTER} {INFO} {sub_site_name}: {sub_url}")
            else:
                print(f"{BEFORE + current_time_hour() + AFTER} {INFO} {site_name}: {url}")

except Exception as e:
    print(f"{BEFORE + current_time_hour() + AFTER} {INFO} Erreur lors de l'exécution du programme: {e}")

# Appel à la fonction qui relance main.py après l'exécution
return_to_main()
