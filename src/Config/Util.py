...
def ChoiceMultiChannelDiscord():
    try:
        num_channels = int(input(f"{BEFORE + current_time_hour() + AFTER} {INPUT} How many spam channels -> {green}"))
    except:
        ErrorNumber()
    ...
    for _ in range(num_channels):
        try:
            number += 1
            selected_channel_number = input(f"{BEFORE + current_time_hour() + AFTER} {INPUT} Channel Id {number}/{num_channels} -> {green}")
